...
export const dragEventSlice = createSlice({
  ...
  reducers: {
    ...
    // be altered
    SET_DRAGGED_HAND_CARD: (state, action: PayloadAction<{draggedCardId: string, gameSnapshot: GameSnapshot }>) => {
      ...
      state.draggedHandCard = gameSnapshot.players[0].places.hand.cards.find(e => e.id === id);
    },
    ...
    SET_HIGHLIGHTS: (state, action: PayloadAction<GameSnapshot>) => {
      ...
      if (draggedHandCard) {
        ...
        const legalTargetIds = actionResults.map(ar => ar.targetId);
        state.highlights = getHighlights(draggedHandCard, gameSnapshot);
