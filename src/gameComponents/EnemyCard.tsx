...
const EnemyCard = (props: EnemyCardProps) => {
    ...
    const {
        ...
        cardHeight,
        cardWidth,
    } = getCardStyleValues(id, currSnapshot);
    const { highlights, BFFdraggedOverSide, draggedOver, draggedHandCard } = useSelector(
        (state: RootState) => state.dragEventState
    );
    const highlightTypeIsCard = useSelector(
        (state: RootState) => state.dragEventState.highlightType === "card"
    );
    const droppableId = JSON.stringify({ id, type: "card" });
    const isDraggedOver = useSelector((state: RootState) => draggedOver?.id === id);
    const notAmongHighlights =
        (highlightTypeIsCard && !highlights.includes(id)) || props.showNotAmongHighlights;
                            ...
                            render={(cardRef, handleClick, handleMouseLeave, inspectingStyles) => (
                                <div ref={cardRef}>
                                    <img
                                        ...
                                        style={{
                                            WebkitFilter: notAmongHighlights
                                                ? "grayscale(100%)"
                                                : "",
