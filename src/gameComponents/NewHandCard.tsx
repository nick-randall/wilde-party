...
import { dimensionConstants, getCardIsFaceup, getCardStyles, getCardStyleValues } from "../helperFunctions/getCardStyles";
...
export const NewHandCard: React.FC<NewHandCardProps> = ({ id, index, imageName, hover, setHover, numHandCards }) => {
  ...
  //   (state: RootState) => state.dragEventState.draggedHandCard !== undefined && state.dragEventState.draggedHandCard.id === id
  // );
  ...
  const leftSpreadCorrection = hover || draggedHandCard?.id === id ? (numHandCards - 1) * dimensionConstants.MIN_HAND_CARD_LEFT_SPREAD: 0
